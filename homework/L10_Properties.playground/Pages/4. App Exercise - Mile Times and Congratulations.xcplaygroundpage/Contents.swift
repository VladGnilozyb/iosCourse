...
struct RunningWorkout {
    ...
    var avegrageMileTime: Double {
        return (distance / 1600) / time
    }
}
var someRunningWorkout = RunningWorkout(distance: 1600, time: 10, elevation: 12)
print(someRunningWorkout.avegrageMileTime)
/*:
...
struct Steps {
    var steps: Int {
        didSet { // willSet вызывается до хранения, как тогда проверить с помощью willSet
            if steps == goal {
                print("congratulations!! you have achieved the goal")
            }
        }
    }
    var goal: Int
...
someSteps.takeStep()
//: [Previous](@previous)  |  page 4 of 6  |  [Next: Exercise - Type Properties and Methods](@next)
